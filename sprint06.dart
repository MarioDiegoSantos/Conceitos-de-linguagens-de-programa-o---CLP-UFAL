...
class Pagamentos {
  ...
  double get saldo_debito => num_saldo_debito;
  void set saldo_debito (double value) => num_saldo_debito = value;

  double get saldo_credito => num_saldo_credito;
  void set saldo_credito (double value) => num_saldo_credito = value;
  ...
  void pagarboleto(int dia, List<int> number, double valor) {
    num_saldo_debito -= num_valor;
    print('Nome do cliente: ${nome_cliente}');
    print('Cpf: ${cpf_cliente.replaceRange(3, 11,'#######')}');
    print('Valor do boleto: ${num_valor}');
    print('Número do boleto: 00197.${ (now.day + now.month + now.year) - dia}7 ${number.toString().replaceAll('[', '').replaceAll(']', '').replaceAll(',', '').replaceAll(' ', '')} 00219.$valor $conta_cliente.0007');
    print('Saldo após o efetuamento do boleto: ${num_saldo_debito}');
  ...
  void pagarpix(aux) {
    (saldo_debito == true) ? num_saldo_debito -= num_valor : num_saldo_debito += num_valor;
    print('Nome do cliente: ${nome_cliente}');
    print('Cpf: ${cpf_cliente.replaceRange(3, 11,'#######')}');
    print('Valor do pix: ${num_valor}');
    print('Chave Pix: ${getRandomString(16)}');
    if(aux == true) print('Saldo após o pagamento pix: ${num_saldo_debito - num_valor}');
    else print('Saldo após o deposito pix: ${num_saldo_debito + num_valor}');
    print("****************************************");
  }
  ...
  void pagarcredito(){
    print('Nome do cliente: ${nome_cliente}');
    print('Cpf: ${cpf_cliente.replaceRange(3, 11,'#######')}');
    print('Valor do pagamento: ${num_valor}');
    print("Limite após o pagamento: ${num_saldo_credito - num_valor}");
    print("****************************************");

    num_saldo_credito -= num_valor;
  }
}
  ...
  if(pagamento == 1){
    ...
    switch(escolha){
     ...
     case 3:
        print('Quanto você quer receber de deposito nesse Boleto?');
        cliente.num_valor = double.parse(stdin.readLineSync()!); 
        cliente.gerarboleto(0,l,cliente.valor); break;
    }
  }
}
...
void pagamento(Pagamentos cliente)
{
  ...
  if(escolha_pagamento == 1){ 
    ...
    if(escolha == 1){  // pix
      ...
      print('Quanto você quer pagar nesse Pix?');
      cliente.num_valor = double.parse(stdin.readLineSync()!);
      if(cliente.num_saldo_debito - cliente.num_valor < -100) print("Saldo insuficiente!, seu saldo após o pagamento ultrapassa o limite de -100!");
      else cliente.pagarpix(aux);
    }
  }

  else if(escolha_pagamento == 2){ // boleto
    print('Agendar => [1]');
    print('Agora => [2]');
    ...
    if(escolha == 1){
      ...
      cliente.num_valor = double.parse(stdin.readLineSync()!);
      if(cliente.num_saldo_debito - cliente.num_valor < -100) print("Saldo insuficiente!, seu saldo após o pagamento ultrapassa o limite de -100!");
      else cliente.pagarboleto(0,l,cliente.valor);
    }
  }

  else if(escolha_pagamento == 3){ // credito
...
void main() {
  ...
  */
  ...
  while(true){
    ...
    switch(escolha){
      case 1: gerar_boleto(cliente); break;
      case 2: pagamento(cliente); break;
      case 3: deposito(cliente); break;
      case 4: exit(exitCode);
    }
  }
}